...
Disclaimer:
You can redistribute this file as you wish, but ALWAYS keeping the name of the
author and a link to http://forum.sa-mp.com/index.php?topic=143025.0
attached to the distribution mean.
For example, the link to the author's name in a public forum thread, or a
README file separated into a .zip file, etc.
If you modify this file, the same terms apply. You have to include the original
author (Zamaroht), the name of the author who continued (BitSain) and the link to the mentioned page.
...
#define ZTDE_INCLUDE_VERSION "v4.0.1"
...
public OnFilterScriptInit() {
	print("\n--------------------------------------");
	print("[TDE]: TextDraw Editor 1.0RC2 by Zamaroht for SA-MP 0.3 Loaded.");
	print("[TDE]: Continued by BitSain (Bug Fixes, Optization, Improvements and languague support)");
	print("[TDE]: Zamaroht's TextDraw Editor Version: "#ZTDE_INCLUDE_VERSION"");
	print("--------------------------------------\n");
...
public OnFilterScriptExit() {
	print("[TDE]: Unloading Zamaroht's TextDraw Editor... (Continued by BitSain)");
    for(new i; i < MAX_TEXTDRAWS; i ++) {
	    TextDrawHideForAll(tData[i][T_Handler]);
	    TextDrawDestroy(tData[i][T_Handler]);
	}
	print("[TDE]: Zamaroht's TextDraw Editor Unloaded. (Continued by BitSain)");
	return true;
}
...
public OnPlayerSpawn(playerid) {
	SendClientMessage(playerid, MSG_COLOR, "[TDE]: Zamaroht's TextDraw Editor "#ZTDE_INCLUDE_VERSION"| Continued by BitSain.");
	SendClientMessage(playerid, MSG_COLOR, "[TDE]: Use /text to show the Edition Menu");
	return true;
}
...
public OnPlayerCommandText(playerid, cmdtext[]) {
	if(!strcmp("/text", cmdtext)){
		if(pData[playerid][P_Editing]) return SendClientMessage(playerid, MSG_COLOR, "[TDE ERROR]: Finish the current edition before using /text!");
		    ...
		    if(IsPlayerMinID(playerid)){
		        ...
		        SendClientMessage(playerid, MSG_COLOR, "[TDE]: Just the smaller player ID can manage projects. Ask him to open one.");
		    return true;
		}
		else {
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
    ...
    switch(dialogid){
        ...
        case 1575: {// New Project
            if(response){
                if(strlen(inputtext) > 120) ShowTextDrawDialog(playerid, 1, 1); // Too long.
					...
					strfind(inputtext, "/") != -1 || strfind(inputtext, "\\") != -1 || strfind(inputtext, ":") != -1 ||
					strfind(inputtext, "*") != -1 || strfind(inputtext, "?") != -1 || strfind(inputtext, "\"") != -1 ||
					strfind(inputtext, "<") != -1 || strfind(inputtext, ">") != -1 || strfind(inputtext, "|") != -1 || 
					!strlen(inputtext) || inputtext[0] == ' ' ) ShowTextDrawDialog(playerid, 1, 3); // Ilegal characters.
            ...
            else ShowTextDrawDialog(playerid, 0);
        }
        ...
        case 1579: {// Main edition menu
            ...
            else ShowTextDrawDialog(playerid, 4, 0);
        }
        ...
        case 1591: {// Change textdraw's font 
            if(response) {
                ...
                if(listitem == 4) {
                    SendClientMessage(playerid,-1, "[TDE]: To use font 4, you have to active the box.");
                    SendClientMessage(playerid,-1, "[TDE]: Change the box size to change TD size.");
                    SendClientMessage(playerid,-1, "[TDE]: Function added by irinel1996.");
				}
                if(listitem == 5) {
        ...
        case 1609: {// Rot X
			if(response) {
                ...
                new string[128];
                format(string, sizeof(string), "[TDE]: Textdraw #%d's changed Preview Model RX to \"%f\".", pData[playerid][P_CurrentTextdraw], floatstr(inputtext));
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 33);
			}
			else
        ...
        case 1610: {// Rot Y
			if(response) {
                ...
                new string[128];
                format(string, sizeof(string), "[TDE]: Textdraw #%d's changed Preview Model RY to \"%f\".", pData[playerid][P_CurrentTextdraw], floatstr(inputtext));
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 33);
			}
			else
        ...
        case 1611: {// Rot Z
			if(response) {
                ...
                new string[128];
                format(string, sizeof(string), "[TDE]: Textdraw #%d's changed Preview Model RZ to \"%f\".", pData[playerid][P_CurrentTextdraw], floatstr(inputtext));
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 33);
			}
			else
        ...
        case 1613: {// Change textdraw's mode
            if(response) {
                ...
                new string[128];
                format(string, sizeof(string), "[TDE]: Textdraw #%d's mode is GLOBAL.", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 5);
            }
            else {
                ...
                new string[128];
                format(string, sizeof(string), "[TDE]: Textdraw #%d's mode is PLAYER", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 5);
            }
        }
...
public ShowTextDrawDialogEx(playerid, dialogid) return ShowTextDrawDialog(playerid, dialogid);
	...
	switch(dialogid) {
	    ...
	    case 33: {
	        if(GetPVarInt(playerid, "Use2DTD") == 1) {
				...
				SendClientMessage(playerid, -1, "[TDE]: Select Font #5 for create a preview TextDraw");
				ShowTextDrawDialog(playerid, 5);
			}
	        return true;
	    }
...
public KeyEdit(playerid) {
	...
	*/
	if(pData[playerid][P_KeyEdition] == EDIT_NONE) return false;
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
    if(pData[playerid][P_KeyEdition] != EDIT_NONE && newkeys == KEY_SECONDARY_ATTACK) {
	    ...
	    switch(pData[playerid][P_KeyEdition]) {
	        case EDIT_POSITION: {
				format(string, sizeof(string), "[TDE]: Textdraw #%d successfuly moved.", pData[playerid][P_CurrentTextdraw]);
	        }
	        case EDIT_SIZE: {
				format(string, sizeof(string), "[TDE]: Textdraw #%d successfuly resized.", pData[playerid][P_CurrentTextdraw]);
	        }
	        case EDIT_BOX: {
				format(string, sizeof(string), "[TDE]: Textdraw #%d's box successfuly resized.", pData[playerid][P_CurrentTextdraw]);
	        }
	    }
...
stock SaveTDData(tdid, const data[]) {
	...
	*/
	...
	if(!strcmp("T_Created", data))
	    ...
	    SendClientMessageToAll(MSG_COLOR, "[TDE]: Incorrect data parsed, textdraw autosave failed");
}
...
stock ExportProject(playerid, type) {
	...
	switch(type){
		case 0: {// Classic export
			...
			format(tmpstring, sizeof(tmpstring), "[TDE]: Project exported to %s.txt in scriptfiles directory.", CurrentProject);
	    ...
	    case 1: {// Show all the time
			...
			format(tmpstring, sizeof(tmpstring), "[TDE]: Project exported to %s.pwn in scriptfiles directory as a filterscript.", CurrentProject);
	        SendClientMessage(playerid, MSG_COLOR, tmpstring);
	    }
	    
	    case 2: {// Show on class selection
            fwrite(File, "/*\r\n");
	        fwrite(File, "Filterscript generated using Zamaroht's TextDraw Editor Version "#ZTDE_INCLUDE_VERSION".\r\n");
	        fwrite(File, "Designed for SA-MP 0.3.\r\n\r\n");
	        new ye,mo,da,ho,mi,se;
	        getdate(ye,mo,da);
	        gettime(ho,mi,se);
			format(tmpstring, sizeof(tmpstring), "Time and Date: %d-%d-%d @ %d:%d:%d\r\n\r\n", ye, mo, da, ho, mi, se);
	    ...
	    case 7: {// PlayerTextDraw by adri1.
	        fwrite(File, "// TextDraw developed using Zamaroht's Textdraw Editor "#ZTDE_INCLUDE_VERSION"\r\n\r\n");
